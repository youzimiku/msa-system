# -*- coding: utf-8 -*-
"""Step4：需求4 补齐 linear/stability/cgcgk/resolution 模拟数据"""
import io
...
def gen_lin(ref, n=12, amp=0.004, seed=0):
    out = []
    for i in range(n):
        v = ref + (-amp + amp*2*( (i*3+seed) % 5 )/5.0 )
...
def gen_stab(n=25, per=5, ref=10.0, amp=0.003, seed=0):
    ...
    for g in range(n):
        row = []
        for i in range(per):
            v = ref + (-amp + amp*2*((g*2+i+seed) % 7)/7.0)
...
def gen_cg(n=50, ref=50.0, amp=0.002, bias=0.0, seed=0):
    out = []
    for i in range(n):
        v = ref + bias + (-amp + amp*2*((i*5+seed) % 9)/9.0)
        out.append(round(v, 4))
    return out

# linear
lin1_raw = [gen_lin(r, 12, 0.004, seed=idx) for idx, r in enumerate([10, 40, 80, 120, 150])]
lin_recs = [
    """{id:'LIN-2026-001', planId:'MSAP-2026-001', instId:'JJQ-2024-001', instName:'数显卡尺 0~150mm', object:'轴径 φ50±0.05', unit:'mm',
     ...
     refs:['10','40','80','120','150'], raw:%s, tolerance:{has:true, usl:50.05, lsl:49.95},
    ...
    {id:'LIN-2026-002', planId:'MSAP-2026-004', instId:'JJQ-2024-002', instName:'外径千分尺 0~25mm', object:'轴径 φ10±0.02', unit:'mm',
     ...
     note:'已建立台账记录，待录入 5 标准件×12 次测量数据后自动完成线性回归与偏倚判定'},""" % str(lin1_raw)
]

# stability
stb1_raw = gen_stab(25, 5, 10.0, 0.003, seed=1)
stb_recs = [
    """{id:'STB-2026-001', planId:'MSAP-2026-004', instId:'JJQ-2024-002', instName:'外径千分尺 0~25mm', object:'轴径 φ10±0.02', unit:'mm',
     ...
     raw:%s, tolerance:{has:true, usl:10.02, lsl:9.98},
    ...
    {id:'STB-2026-002', planId:'MSAP-2026-003', instId:'JJQ-2024-007', instName:'数显扭力扳手 5~50N·m', object:'螺栓拧紧力矩 25N·m', unit:'N·m',
     ...
     analysisDate:'', analyst:'', reviewStatus:'待采集', conclusion:'', reviewer:'', reviewDate:'', approver:'', approveDate:'', actions:[],
     note:'已建立台账记录，待按子组录入测量数据后自动绘制控制图并判异'},""" % str(stb1_raw)
]

# cgcgk
cg1_raw = gen_cg(50, 50.0, 0.002, 0.0, seed=2)
cg2_raw = gen_cg(50, 25.0, 0.05, 0.05, seed=3)
cg_recs = [
    """{id:'CG-2026-001', planId:'MSAP-2026-001', instId:'JJQ-2024-001', instName:'数显卡尺 0~150mm', object:'轴径 φ50±0.05', unit:'mm',
     ...
     raw:%s, tolerance:{has:true, usl:50.05, lsl:49.95},
    ...
    {id:'CG-2026-002', planId:'MSAP-2026-003', instId:'JJQ-2024-007', instName:'数显扭力扳手 5~50N·m', object:'螺栓拧紧力矩 25N·m', unit:'N·m',
     ...
     raw:%s, tolerance:{has:true, usl:27.5, lsl:22.5},
    ...
    {id:'CG-2026-003', planId:'MSAP-2026-004', instId:'JJQ-2024-002', instName:'外径千分尺 0~25mm', object:'轴径 φ10±0.02', unit:'mm',
     ...
     analysisDate:'', analyst:'', reviewStatus:'待采集', conclusion:'', reviewer:'', reviewDate:'', approver:'', approveDate:'', actions:[],
     note:'已建立台账记录，待录入 50 次连续测量并填写参考值后自动计算 Cg/Cgk'},""" % (str(cg1_raw), str(cg2_raw))
]

# resolution
res_recs = [
    """{id:'RES-2026-001', planId:'MSAP-2026-001', instId:'JJQ-2024-001', instName:'数显卡尺 0~150mm', object:'轴径 φ50±0.05', unit:'mm',
